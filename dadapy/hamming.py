...
class Hamming:
    ...
    def _set_cross_distances(
        ...
        if "JAX_PLATFORMS" not in os.environ:
            ...
            if self.verbose:
                print("running JAX on GPU")
        else:
            os.environ["JAX_PLATFORMS"] = "cpu"
            self.crossed_distances = 0
            if self.verbose:
                print("running JAX on CPU")
...
def jcompute_distances(
    ...
    pytree = lax.fori_loop(lower=0, upper=Ns1, body_fun=_jcompute_distances, init_val=pytree)
...
def _jcompute_distances(idx, pytree):
    ...
    pytree = lax.cond(pytree["crossed_distances"], _set_lower_idx_true, _set_lower_idx_false, pytree)
...
def subsample_data(coordinates, N1=None, seed=0):
    ...
    indices_rows = random.choice(subkey, jnp.arange(coordinates.shape[0]), shape=(N1,), replace=False)
    indices_columns = jnp.setdiff1d(jnp.arange(coordinates.shape[0]), indices_rows, assume_unique=True)
...
class BID:
    def __init__(
        ...
        alphamin=0.05,
        alphamax=0.4,
        ds=jnp.zeros(shape=2, dtype=jnp.float64),
        steps_initial=jnp.array([1e-1], dtype=jnp.float64),  # amplitude modulating the gradient descent step
        step_final=jnp.float64(1e-4),
        Nsteps=jnp.int32(
            1e3
        ),  # total number of optimization steps. There is an early stopping condition, so the optimization can stop before Nsteps.
    ...
    def truncate_hist(self):
        self.remp = jnp.array(self.H.D_values[self.idmin : self.idmax + 1], dtype=jnp.float64)
        self.Pemp = jnp.array(self.H.D_probs[self.idmin : self.idmax + 1], dtype=jnp.float64)
    ...
    def computeBID(
        ...
        if self.export_results:
            ...
            np.savetxt(
                fname=self.valfile,
                X=np.transpose([self.remp, self.Pemp, self.opt.vars["Pmodel"]]),
            )
            if self.export_logKLs:
                np.savetxt(fname=self.KLfile, X=self.opt.vars["logKLs"])
...
class Optimizer(struct.PyTreeNode):
    ...
    def create(
        ...
        vars["ds"] = jnp.zeros(shape=(2), dtype=jnp.float64)
        vars["step"] = jnp.float64(0)
        vars["grad"] = jnp.zeros(shape=(2), dtype=jnp.float64)
        vars["Hessian"] = jnp.zeros(shape=(2, 2), dtype=jnp.float64)
        vars["logKL"] = jnp.float64(0.0)
        ...
        vars["logKLs"] = jnp.zeros(shape=(params["Nsteps"]), dtype=jnp.float64)
        vars["ds_optimization"] = jnp.zeros(shape=(params["Nsteps"], len(vars["ds"])))

        ### EARLY STOPPING
        vars["counter_early_stopping"] = jnp.int32(0)
        params["threshold_early_stopping"] = jnp.float64(1e-5)
        params["tolerance_early_stopping"] = jnp.int32(200)
        vars["early_stopping_condition"] = False
        vars["final_optimization_step"] = jnp.int32(-1)

        ### INITIAL CONDITION
        params["auxmin"] = jnp.float64(0.05)
        params["auxmax"] = jnp.float64(0.95)
        params["auxstep"] = jnp.float64(0.05)
        params["d00_list"] = jnp.arange(
            params["auxmin"], params["auxmax"] + params["eps"], params["auxstep"], dtype=jnp.float64
        )
        params["d10_list"] = jnp.float64(2) - params["L"] * params["d00_list"] / jnp.dot(
            params["remp"], params["Pemp"]
        )  # smart initial condtion from Cristopher Erazo

        # possibly useless, but previously useful guess:
        params["d10_list"] = jnp.concat([params["d10_list"], jnp.ones_like(params["d00_list"])])
        params["d00_list"] = jnp.concatenate([params["d00_list"], params["d00_list"]])

        vars["logKLs0"] = jnp.empty(shape=params["d00_list"].shape[0], dtype=jnp.float64)
        return cls(
            params,
            vars,
        )
...
def test_initial_conditions(opt):
    ...
    opt = lax.fori_loop(lower=0, upper=opt.vars["logKLs0"].shape[0], body_fun=_test_initial_conditions, init_val=opt)

    opt.vars["logKLs0"] = jnp.where(
        jnp.isnan(opt.vars["logKLs0"]),
        jnp.inf,
        opt.vars["logKLs0"],
    )

    top_k_initial_conditions = lax.top_k(-opt.vars["logKLs0"], k=opt.vars["indices_optimization"].shape[0])
...
def _logPmodel(opt):
    ...
    opt.vars["logPmodel"] = -opt.vars["d_of_r"] * jnp.log(jnp.float64(2))
    opt.vars["logPmodel"] += gammaln(opt.vars["d_of_r"] + jnp.float64(1))
    opt.vars["logPmodel"] -= gammaln(opt.vars["d_of_r"] - opt.params["remp"] + jnp.float64(1))
    opt.vars["logPmodel"] -= gammaln(opt.params["remp"] + jnp.float64(1))
...
def clip_grad(ds, gradient):
    return jnp.clip(
        gradient,
        min=-jnp.array(ds * 5.0 / 100, dtype=jnp.float64),
        max=jnp.array(ds * 5.0 / 100, dtype=jnp.float64),
    )
...
def _optimization_step(idx, opt):
    ...
    opt.vars["grad"] = lax.cond(
        pred=idx < opt.params["Nsteps"] // 2,
    ...
    opt.vars["logKLs"] = (
        opt.vars["logKLs"].at[idx].set(jnp.log(loss_val + opt.params["Pemp"] @ jnp.log(opt.params["Pemp"])))
    )
    opt.vars["ds_optimization"] = opt.vars["ds_optimization"].at[idx].set(opt.vars["ds"])

    # early stopping:
    pred = abs_relative_change(opt.vars["ds"][0], opt.vars["ds_optimization"][(idx - 1) % opt.params["Nsteps"], 0])
...
def replace_nans_with_ones(float_64_tensor):
    return jnp.where(jnp.isnan(float_64_tensor), jnp.float64(1.0), float_64_tensor)


def step_schedule(idx, opt):
    opt.vars["step"] = opt.params["step_initial"] + jnp.float64(idx) / opt.params["Nsteps"] * (
        opt.params["step_final"] - opt.params["step_initial"]
    )
    return opt
...
def minimize_loss(opt):
    ...
    opt = lax.fori_loop(lower=0, upper=opt.params["Nsteps"], body_fun=early_stopping_optimization_step, init_val=opt)
...
def early_stopping_optimization_step(idx, opt):
    ...
    return lax.cond(
        opt.vars["early_stopping_condition"], lambda _: do_nothing_idx(*_), lambda _: _optimization_step(*_), (idx, opt)
    )
...
def compute_KL(opt):
    opt = d_of_r(opt, opt.vars["ds"])
    opt = _logPmodel(opt)
    KL = jnp.dot(opt.params["Pemp"], jnp.log(opt.params["Pemp"]) - jnp.log(opt.vars["Pmodel"]))
